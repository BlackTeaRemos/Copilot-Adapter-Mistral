...
function extractText ( part: LanguageModelTextPart | LanguageModelToolCallPart | LanguageModelToolResultPart ): string {
...
export class MistralChatModelProvider implements LanguageModelChatProvider {
    ...
    private authDeps (): AuthDeps {
        return {
            ...
            invalidateModelCache: () => { this.fetchedModels = null; },
            fireModelInfoChange: () => { this._onDidChangeLanguageModelChatInformation.fire( undefined ); },
            validateApiKey: apiKey => this.validateApiKey( apiKey ),
        };
    }
    ...
    async provideLanguageModelChatInformation (
        ...
        if ( !initialized ) { this.log.warn( '[Mistral] client not initialized' ); return []; }
        if ( token.isCancellationRequested ) { return []; }
