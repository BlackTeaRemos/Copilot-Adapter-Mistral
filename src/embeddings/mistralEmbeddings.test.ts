    ...
    return {
        embeddings: {
            create: vi.fn( async ( req: { inputs: string[]; } ) => ( {
                data: handler( req.inputs ).map( ( embedding, index ) => ( { embedding, index } ) ),
            } ) ),
        },
    } as any;
}
...
describe( 'rankBySimilarity', () => {
    ...
    it( 'drops items at or below minScore', () => {
        ...
        expect( r.map( x => x.item ) ).toEqual( [ 'a', 'b' ] );
        expect( r.some( x => x.item === 'c' ) ).toBe( false );
    } );
} );

describe( 'createEmbeddings', () => {
    it( 'returns one vector per input, index-aligned', async () => {
        const client = mockClient( inputs => inputs.map( ( _, i ) => [ i, i + 1 ] ) );
    ...
    it( 'batches large inputs', async () => {
        const client = mockClient( inputs => inputs.map( () => [ 1 ] ) );
        const inputs = Array.from( { length: EMBED_BATCH_SIZE + 5 }, ( _, i ) => `t${ i }` );
    ...
    it( 'reports progress and stops when cancelled', async () => {
        const client = mockClient( inputs => inputs.map( () => [ 1 ] ) );
        const inputs = Array.from( { length: EMBED_BATCH_SIZE * 3 }, ( _, i ) => `t${ i }` );
        let cancel = false;
        const seen: number[] = [];
        await createEmbeddings( client, 'mistral-embed', inputs, {
            onProgress: done => { seen.push( done ); cancel = true; },
            shouldCancel: () => cancel,
        } );
