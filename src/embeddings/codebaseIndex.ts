...
export function planReindex(
    ...
    for ( const { file, hash } of current ) {
        seen.add( file );
        if ( cached[ file ] && cached[ file ].hash === hash ) {
            reuse.push( file );
        } else {
            embed.push( file );
        }
    }
    const remove = Object.keys( cached ).filter( f => {
        return !seen.has( f );
    } );
        ...
        try {
            ...
            for ( const uri of found ) {
                if ( token?.isCancellationRequested ) {
                    this.setState( this.chunkCount > 0 ? `ready` : `empty` ); return this.result( 0, 0 );
                }
                try {
            ...
            const reusedChunks = plan.reuse.reduce( ( n, f ) => {
                return n + ( this.files[ f ]?.chunks.length ?? 0 );
            }, 0 );
            ...
            for ( const file of plan.embed ) {
                ...
                for ( const c of chunkDocument( entry.text ) ) {
                    pending.push( { ...c, file } );
                }
            }
            ...
            if ( pending.length > 0 ) {
                ...
                const vectors = await createEmbeddings( client, model, pending.map( c => {
                    return c.text;
                } ), {
                    shouldCancel: () => {
                    ...
                    onProgress: ( done, total ) => {
                        return progress?.report( { message: `Embedding ${ done }/${ total } changed chunks…` } );
                    },
                    log: this.log,
                } );
